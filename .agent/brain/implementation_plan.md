...
### Updated Pipeline
1.  **Identify Online Stores**: Query the `stores` table for names matching "ליקוט", "Online", or specific known codes.
2.  **Geocoding**: During the store sync, we'll convert the branch addresses into Latitude/Longitude coordinates using the Google Maps or OpenStreetMap API.
3.  **PostGIS Integration**: We will enable the `postgis` extension in Supabase to store coordinates in a `geography` column.
4.  **Nearest Branch Matching**: On the frontend, when a user enters their location, the app will execute a "Nearest Neighbor" SQL query to find the specific Online/Likot branch for each chain that serves their area.

## Plan
1.  **Setup**: [DONE] Initialize Next.js app and Supabase project.
2.  **Data Ingestion**: 
    - [DONE] Build core sync engine in `backend/main.py`.
    - [DONE] Sync Tiv Taam baseline.
    - [ ] Update engine to support multi-store "Online" sync.
    - [ ] Add **Geocoding** step to the ingestion pipeline.
    - [ ] Implement distance-based store matching using **PostGIS**.
